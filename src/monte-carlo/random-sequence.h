...
public:
    RandomSequence(RNG&& r, const RangeInfinite<Number>& ra = RangeInfinite<Number>()) : 
...
auto random_sequence(const RangeInfinite<Number>& r, std::size_t seed = std::random_device()()) {
    return RandomSequence<RNG,Number>(RNG(seed),r);
} 

/*
template<typename Number,typename RNG>
auto random_sequence(const RNG& rng, const RangeInfinite<Number>& r) {
    return RandomSequence<RNG,Number>(rng,r);
} 
*/
