...
auto integrator_adaptive_fubini_variance_reduction(const R& rule, const EH& error_heuristic, std::size_t iterations, unsigned long mc_samples, RR&& rr, CV&& cv, RS&& region_sampler, RNG&& rng, unsigned long spp, std::size_t nmutexes = 16) {
    RNG rn = rng;
    return integrator_region_based(
        ...
        regions_integrator_variance_reduction(std::forward<RR>(rr),std::forward<CV>(cv), std::forward<RS>(region_sampler), rn,spp,nmutexes));
}

template<std::size_t N, typename RR, typename CV, typename RS, typename R, typename EH>
auto integrator_adaptive_fubini_variance_reduction(const R& rule, const EH& error_heuristic, std::size_t iterations, unsigned long mc_samples, 
    ...
    return integrator_region_based(
        ...
        regions_integrator_variance_reduction(std::forward<RR>(rr),std::forward<CV>(cv), std::forward<RS>(region_sampler), std::mt19937(seed+1),spp,nmutexes));
}
